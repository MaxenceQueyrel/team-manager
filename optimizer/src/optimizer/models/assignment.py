...
class AssignmentWeights(BaseModel):
    ...
    handover: float = Field(
        ...
        ge=0,
        le=1,
        description="Reward for keeping the same people across consecutive phases. 0 solves phases independently.",
    )
...
class AssignmentResult(BaseModel):
    ...
    max_score: float = Field(
        description="Theoretical maximum score achievable for this assignment's weights and slot count."
    )
